...
public class cutRNNmipro extends SingleInputStreamOperator {
	...
	public cutRNNmipro(int operatorID, StreamQueue[] input,
			StreamQueue[] output) {
		...
		for (int i = maxDequeueSize; i >= 0; i--) {
			...
			if (w1.isfull()) {
				...
				*/
				...
				execution_Start = (new Date()).getTime(); // start time. // /yyw
				rnnmap = (new cutRNNmipro_function(w1, k, n)).RNN_determine_all();
