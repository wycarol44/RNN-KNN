...
public class CopyOfupdateBFlifespanRNN2 extends SingleInputStreamOperator {
	...
	public CopyOfupdateBFlifespanRNN2(int operatorID, StreamQueue[] input,
			StreamQueue[] output) {
	...
	public int run(int maxDequeueSize) {
		...
		for (int i = maxDequeueSize; i >= 0; i--) {
			...
			if (w1.isfull()) {
				...
				if (!w1.isInit()) {
					rnnmap = new CopyOfupdateBFlifespanRNN2_function(w1, k, n)
