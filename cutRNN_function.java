...
public class cutRNN_function {
	...
	public cutRNN_function(Window_Container2 w, int k, int n) {
		this.k = k;
		this.n = n;
		this.w = w;
...
/*
	...
	public HashMap RNN_determine_onetuple(Tuple_container queryinput,int count,int n) {
...
//run nl_cut_0_0.xml
		...
		while(determined==false&&countno!=w.getRange()-1) {
			...
			*/
			...
			orderdist = Math.abs(w.getKTupleDist_g(i)
					- w.getKTupleDist_g(0));
			...
			if (!stoprule||count<=k+1) {
				...
				if (Isrnn) {
					...
					initialize_original(w.getTuple_g(i));
					knnvalue = w.getTuple_g(i).getK();
